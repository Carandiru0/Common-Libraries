...
constinit static inline uint16_t const srgb_to_linear_lut[256] = { // single channel, 10bit linear returned from input 8bit srgb color.
	0,
	0,
	1,
	1,
...
{
	uvec4_t srgb;
	SFM::unpack_rgba(packed_srgb, srgb);   // 8bpc
...
uint32_t const ImagingSRGBtoLinear(uint32_t const packed_srgb)
{
	...
	return(SFM::pack_rgb_hdr(linear));   // 10bpc
}
