...
// *** modified to use no lock and wide character variants of file stream functions, _CRT_DISABLE_PERFCRIT_LOCKS is defined globally in project properties ***
// *** modified to use scalable allocations
...
static int InternalRead(GifFileType *gif, GifByteType *buf, int len) {
    ...
    return
        (((GifFilePrivateType*)gif->Private)->Read ?
            ...
            _fread_nolock_s(buf, len, 1, len, ((GifFilePrivateType*)gif->Private)->File));
}
...
******************************************************************************/
GifFileType *
DGifOpenFileName(wchar_t const* const FileName, int *Error)
{
    int FileHandle = -1;
    GifFileType *GifFile;

    if (0 != _wsopen_s(&FileHandle, FileName, _O_RDONLY | _O_BINARY | _O_SEQUENTIAL, _SH_DENYNO, _S_IREAD) || -1 == FileHandle) {
        if (Error != NULL) {
...
DGifOpenFileHandle(int FileHandle, int *Error)
{
    ...
    GifFile = (GifFileType *)scalable_malloc(sizeof(GifFileType));
    if (GifFile == NULL) {
        if (Error != NULL)
    ...
    Private = (GifFilePrivateType *)scalable_calloc(1, sizeof(GifFilePrivateType));
    if (Private == NULL) {
        if (Error != NULL)
	    *Error = D_GIF_ERR_NOT_ENOUGH_MEM;
        (void)_close(FileHandle);
        scalable_free((char *)GifFile);
        return NULL;
    }
    ...
    f = _wfdopen(FileHandle, L"rb");    /* Make it into a stream: */
    ...
    if (InternalRead(GifFile, (unsigned char *)Buf, GIF_STAMP_LEN) != GIF_STAMP_LEN) {
	    ...
	    *Error = D_GIF_ERR_READ_FAILED;
        (void)_fclose_nolock(f);
        scalable_free((char *)Private);
        scalable_free((char *)GifFile);
        return NULL;
    }
...
DGifGetImageDesc(GifFileType *GifFile)
{
    ...
    if (GifFile->SavedImages) {
        ...
        if ((GifFile->SavedImages =
             (SavedImage *)scalable_malloc(sizeof(SavedImage))) == NULL) {
            GifFile->Error = D_GIF_ERR_NOT_ENOUGH_MEM;
            return GIF_ERROR;
        }
    }
...
DGifCloseFile(GifFileType *GifFile, int *ErrorCode)
{
    ...
    scalable_free((char *)GifFile->Private);
    scalable_free(GifFile);
        return GIF_ERROR;
    }

    if (Private->File && (_fclose_nolock(Private->File) != 0)) {
	if (ErrorCode != NULL)
