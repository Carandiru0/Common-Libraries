...
__m256i const __vectorcall Hash(__m256i const data, uint64_t const hashSeed = 0); // upto 4x 64 bit numbers
__m128i const __vectorcall Hash(__m128i const data, uint32_t const hashSeed = 0); // upto 4x 32 bit numbers
...
STATIC_INLINE_PURE __m256i const __vectorcall _rotl256(__m256i const x, int const k) {
...
STATIC_INLINE_PURE __m128i const __vectorcall _rotl128(__m128i const x, int const k) {

	//return((x << k) | (x >> (32 - k)));

	__m128i const xmReg0 = _mm_slli_epi32(x, k);
	__m128i const xmReg1 = _mm_srli_epi32(x, 32 - k);

	return(_mm_or_si128(xmReg0, xmReg1));
}
...
STATIC_INLINE_PURE __m256i const __vectorcall HashState(__m256i const pendingState)
{
	...
	__m256i const rotate(_rotl256(h64, 17 << 1)); // this avoids an illegal instruction being generated by compiler, leave it alone
...
__m256i const __vectorcall Hash(__m256i const data, uint64_t const hashSeed) // upto 4x 64 bit numbers
{
	[[unlikely]] if (!oRandom.Initialized) {
		InitializeRandomNumberGeneratorInstance();
	}
	...
	__m256i h64(_mm256_add_epi64(_mm256_add_epi64(_mm256_set1_epi64x(oRandom.hashSeed), _mm256_set1_epi64x(hashSeed)), _mm256_set1_epi64x(PRIME64_5)));		// this where HashSetSeed has influence on random numbers generated going forward
	h64 = _mm256_add_epi64(h64, _mm256_set1_epi64x(4));
	h64 = _mm256_add_epi64(h64, _mm256_mullo_epi32(data, _mm256_set1_epi64x(PRIME64_3)));
	__m256i const rotate(_rotl256(h64, 17 << 1)); // this avoids an illegal instruction being generated by compiler, leave it alone
	h64 = _mm256_mullo_epi32(rotate, _mm256_set1_epi64x(PRIME64_4));
	h64 = _mm256_xor_si256(h64, _mm256_srli_epi64(h64, 15 << 1));
	h64 = _mm256_mullo_epi32(h64, _mm256_set1_epi64x(PRIME64_2));
...
__m128i const __vectorcall Hash(__m128i const data, uint32_t const hashSeed) // upto 4x 32 bit numbers
{
	[[unlikely]] if (!oRandom.Initialized) {
		InitializeRandomNumberGeneratorInstance();
	}
	...
	__m128i h32(_mm_add_epi32(_mm_add_epi32(_mm_set1_epi32((uint32_t)oRandom.hashSeed), _mm_set1_epi32(hashSeed)), _mm_set1_epi32(PRIME32_5)));		// this where HashSetSeed has influence on random numbers generated going forward
	h32 = _mm_add_epi32(h32, _mm_set1_epi32(4));
	h32 = _mm_add_epi32(h32, _mm_mullo_epi32(data, _mm_set1_epi32(PRIME32_3)));
	__m128i const rotate(_rotl128(h32, 17)); // this avoids an illegal instruction being generated by compiler, leave it alone
	h32 = _mm_mullo_epi32(rotate, _mm_set1_epi32(PRIME32_4));
	h32 = _mm_xor_si128(h32, _mm_srli_epi32(h32, 15));
	h32 = _mm_mullo_epi32(h32, _mm_set1_epi32(PRIME32_2));
	h32 = _mm_xor_si128(h32, _mm_srli_epi32(h32, 13));
	h32 = _mm_mullo_epi32(h32, _mm_set1_epi32(PRIME32_3));
	h32 = _mm_xor_si128(h32, _mm_srli_epi32(h32, 16));
